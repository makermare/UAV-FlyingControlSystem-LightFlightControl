...
float Height_Ctrl(float T,float thr,u8 ready,float en)
{
	...
	if(ABS(en - en_old) > 0.5f)//从非定高切换到定高
	{
	 ...
	 if(mode_state==2)//超声波模式，代表一键起飞定高
	 {
			...
			else if(ultra.height>35&&ultra.height<40)
			{
			   thr=560;
			}
			else if(ultra.height>=40&&ultra.height<45)
			{
			   thr=550;
			}
		...
		if(mode_state==1)//超声波模式标志，代表一键降落
		{
		   thr=300;//下降
		}
	...
	if(thr_set>0)//对应油门遥杆大于1500，想要上升
	{
		...
		if(thr_set>100)//对应油门遥杆1640
		{
			...
			if(!thr_take_off_f)//起飞标志为0
			{
				...
				thr_take_off =400 ; //直接赋值 一次//考虑这里可以直接赋值400;
			}
		}
	}
