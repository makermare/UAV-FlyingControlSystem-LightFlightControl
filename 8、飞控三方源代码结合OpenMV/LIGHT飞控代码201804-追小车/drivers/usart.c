...
#define  Pitch_Compensation    64.0f/(2.0f*atan(16.0f/50.0f)*57.3f)
...
u8 USART3_RXBUF[10];     //接收数据
uint8_t RXOVER3 = 0;     //接收数据完成标志
uint8_t RXCUNT3 = 0;     //接收字节数量
extern u8 start_flag;//开始寻线的标志
extern uint32_t runtime1;
extern float move_front;
extern u8 position_control;
//串口3用来和瑞萨进行双向通信
void USART3_IRQHandler(void)//
{
	...
	if(USART_GetITStatus(USART3,USART_IT_RXNE)!= RESET)//接收到数据
	{
	  USART_ClearITPendingBit(USART3,USART_IT_RXNE);		
		...
		if(temp=='#')//接收数据完成摄像头
		{  
		    if(USART3_RXBUF[0]=='s')//校验帧头正确
				{  
				  if(USART3_RXBUF[1]=='p')
					{
					    if(PixFlow_TrackLine_flag==2&&start_flag==3)
							{
							   start_flag=4;
								 runtime1=0;
							}
