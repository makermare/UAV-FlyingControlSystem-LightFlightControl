...
void h_pid_init()
{
  ...
  //光流位置环PID控制
	y_offset_arg.kp = 1.8f *pid_setup.groups.hc_height.kp;				//比例系数
	...
	//光流位置环PID控制
	x_offset_arg.kp = 1.8f*pid_setup.groups.hc_height.kp;				//比例系数
...
float Height_Ctrl(float T,float thr,u8 ready,float en)
{
	...
	if(ABS(en - en_old) > 0.5f)//从非定高切换到定高
	{
	 ...
	 if(mode_state==2)//气压计模式标志，代表一键起飞定高
	 {
		  ...
		  else if(ultra.height>20&&ultra.height<=30)
		  {
		     thr=650-(ultra.height-20)*10;
      }
		  else if(ultra.height>30&&ultra.height<=40)
		  {
			   thr=551;
		  }
			...
			else if(ultra.height>=50&&ultra.height<=60)
			{
			   thr=450;
			}
