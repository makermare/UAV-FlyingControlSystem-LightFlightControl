...
void h_pid_init()
{
	...
	h_acc_arg.ki = 0.07f  *pid_setup.groups.hc_sp.kp;				//积分系数
	h_acc_arg.kd = 0;				//微分系数
	h_acc_arg.k_pre_d = 0 ;	
	h_acc_arg.inc_hz = 0;
  ...
  //光流位置环PID控制
	...
	//光流位置环PID控制
	x_offset_arg.kp = 0.53f*pid_setup.groups.hc_height.kp;				//比例系数
	x_offset_arg.ki = 0.0f*pid_setup.groups.hc_height.ki;				//积分系数
	x_offset_arg.kd = 0.4f*pid_setup.groups.hc_height.kd;				//微分系数
	...
	if(ABS(en - en_old) > 0.5f)//从非定高切换到定高
	{
	 ...
	 if(mode_state==2)//气压计模式标志，代表一键起飞定高
	 {
		  ...
		  else if(ultra.height>20&&ultra.height<=25)
		  {
			...
			else if(ultra.height>25&&ultra.height<35)
			{
			   thr=550;
			}
			else if(ultra.height>=35&&ultra.height<=45)
			{
			   thr=500;
			}
			else if(ultra.height>45&&ultra.height<60)
			{
				 thr=450;//下降
			}
		...
		if(mode_state==1||start_flag==4)//超声波模式标志，代表一键降落
		{
		    thr=300;//下降
		}
...
/////////////////////////////////////////////////////////////////////////////////	
    ...
    //(2)速度环控制控制周期=20ms，，，注意输入的值为 ：  期望速度+设定速度    ，设定速度也是通过油门值来控制的
		...
		//------------------------------------光流定点控制速度环20ms---------------------------------------------------//
		 ...
		 if((PixFlow_TrackLine_flag==1)&&(ultra.height>20)&&(position_control!=0))
		 {
			 if(position_control==1)//定点控制
			 {
			 ...
			 else if(position_control==2)//只进行横滚控制
			 {
			    exp_speed_x= PID_calculate(dT,            //周期
																0,				//前馈
																0,				//期望值（设定值）
			 ...
			 //exp_speed_x = LIMIT(exp_speed_x,-speed_max,speed_max);
			 //exp_speed_y = LIMIT(exp_speed_y,-speed_max,speed_max);
			 PixFlow_Roll  = LIMIT(exp_speed_x/50.0f,-speed_max,speed_max);
			 PixFlow_Pitch =-LIMIT(exp_speed_y/50.0f,-speed_max,speed_max);
		  }
//		if(PixFlow_TrackLine_flag==1)
//		{
//		  PixFlow_Roll=PID_calculate(dT,      //周期
//														0,	//前馈
//														exp_speed_x,	//期望值（设定值）
//														X_Speed,			//反馈值
//														&x_speed_arg, //PID参数结构体
//														&x_speed_val,	//PID数据结构体
//														500 *en			  //integration limit，积分限幅
//														 );           //输出	
//		  PixFlow_Pitch=PID_calculate(dT,              //周期
//														0,				//前馈
//														exp_speed_y,				//期望值（设定值）
//														Y_Speed,			//反馈值
//														&y_speed_arg, //PID参数结构体
//														&y_speed_val,	//PID数据结构体
//														500*en			//integration limit，积分限幅
//														 );//输出											 
...
//	   //------------------------------------光流定点控制位置环200ms开始---------------------------------------------------//
//		 if((PixFlow_TrackLine_flag==1)&&(ultra.height>25))
//		 {
//			 if(position_control==1)//定点控制
//			 {
//			  exp_speed_x=PID_calculate(dT2,            //周期
//																0,				//前馈
//																0,				//期望值（设定值）
//																X_Offset,			//反馈值
//																&x_offset_arg, //PID参数结构体
...
//																 );    //输出	
//			
//			  exp_speed_y=PID_calculate(dT2,            //周期
//																0,				//前馈
//																0,				//期望值（设定值）
//																Y_Offset,			//反馈值
//																&y_offset_arg, //PID参数结构体
...
//																 );			//输出	
//			 }
//			 else if(position_control==0)//只进行横滚控制
//			 {
//			    exp_speed_x= PID_calculate(dT2,            //周期
//																0,				//前馈
//																0,				//期望值（设定值）
