...
void Track_Line(void)
{
	...
	{  
		 ...
		 if((ultra.height>30)&&(start_flag==0))
		 {
				runtime1++;
				if(runtime1==1000)//2s
				{
		 ...
		 {
				runtime1++;
				if((start_flag==1)&&(runtime1==10))//开始以0.5度倾角向前
				{
					 turn_left=Yaw;
					 turn_flag=0;
					 move_front=-1.0f;     //-1度
				...
				{
				    move_front=1.5f;    //检测到直角就停止前进
					  start_flag=3;
					  position_control=1; //开始进行定点控制
					  runtime1=0;
				}
				else if((start_flag==3)&&(runtime1==30))
				{
					  move_front=0;
					  //position_control=1;//不进行定点控制
				}
				else if((start_flag==3)&&(runtime1>=120)&&(runtime1<=168))//刹车2s,两种方法，要么刹车之后再定点，要么定点和刹车一起进行
				{
					  position_control=1;
					  turn_left=turn_left-(s16)(150*0.5f )*0.025;
					  if(turn_left<-180)
					  {
						  turn_left=360-180;
					  }
				}
				else if((start_flag==3)&&(runtime1==208))//
				{
					   start_flag=1;
					   //turn_flag=1 ;
					   runtime1=0;
