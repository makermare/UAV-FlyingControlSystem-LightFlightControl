...
#define  Pitch_Compensation    32.0f/(2.0f*atan(16.0f/50.0f)*57.3f)

uint8_t USART_RXBUF[20];//缓冲数组
	  ...
	  USART_ClearITPendingBit(USART1,USART_IT_RXNE);		
		...
		{  
		    if(USART_RXBUF[0]=='s')//校验帧头正确
				...
				{
					if(start_flag==2&&my_data[6]==1)//如果start_flag==2表示正在前进，此时需要查询有没有检测到直角转弯的标志
					{
					  turn_flag=1;
					}
					//像素点的位移之和转换成实际的距离,实际测试，B64x32的图片   高度50cm 60cm对应60个像素点
				  no_compen_x_speed=(float)my_data[1]*(float)ultra.height/50.0f/0.02f;
					no_compen_y_speed=(float)my_data[2]*(float)ultra.height/50.0f/0.02f;
					//暂时不查看
					//no_compen_x_offset+=(float)my_data[1]*(float)ultra.height/50.0f;
					//no_compen_y_offset+=(float)my_data[2]*(float)ultra.height/50.0f;
					//暂时不查看
					//X_Offset+=((float)my_data[1]/10.0f-Roll_Compensation*(-mpu6050.Gyro_deg.x)*0.02f)*(float)ultra.height/50.0f*10.f;//单位为mm
					//Y_Offset+=((float)my_data[2]/10.0f-Pitch_Compensation*(-mpu6050.Gyro_deg.y)*0.02f)*(float)ultra.height/50.0f*10.f;//单位为mm    
					//位移暂时用圆心坐标来表示
					X_Offset= (float)(32-my_data[4])*(float)ultra.height/50.0f*10;//单位mm
					Y_Offset=-(float)(16-my_data[5])*(float)ultra.height/50.0f*10;
				  ...
				  //计算对应的速度. my_data[1]=移动像素点*10的倍数  T=0.02s    陀螺仪角速度单位 degree/s 
					X_Speed=((float)my_data[1]/10.0f-Roll_Compensation*(-mpu6050.Gyro_deg.x)*0.02f)*(float)ultra.height/50.0f*10.0f/0.02f;//mm/s 
          Y_Speed=((float)my_data[2]/10.0f-Pitch_Compensation*(-mpu6050.Gyro_deg.y)*0.02f)*(float)ultra.height/50.0f*10.0f/0.02f;
				}
			  RXCUNT = 0;
			  RXOVER = 1;//接收完成标志位置
			 	for(i=0;i<20;i++)
